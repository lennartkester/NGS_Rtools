getWESqcData <- function(){
  ...
  for ( i in 1:length(pairedSamples)){
    curSampleDir <- list.dirs(paste0(baseDirWES,"QualityControl/multiQCfiles/",pairedSamples[i]),recursive = T)[2]
  ...
  for ( i in 1:length(singleSamples)){
    curSampleDir <- list.dirs(paste0(baseDirWES,"QualityControl/multiQCfiles/",singleSamples[i]),recursive = T)[2]
    if (is.na(curSampleDir)){
      next
    }
    tempList <- list()
  ...
  multiQCdata$Status <- "unknown"
  multiQCdata$Status[multiQCdata$`Biomaterial ID` == multiQCdata$pair_sample1] <- "Tumor"
  multiQCdata$Status[multiQCdata$`Biomaterial ID` == multiQCdata$pair_sample2] <- "Normal"
